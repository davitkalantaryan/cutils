...
static inline void RecoverDataAndRegsInline(int a_pid, const struct user_regs_struct* a_pOldRegs, 
											unsigned long long int a_addr, size_t a_itersCount, const unsigned long long int* a_save){
	size_t itr;
	/* Revert the registers, too, to the old state. */
	ptrace(PTRACE_SETREGS, (pid_t)a_pid, CPPUTILS_NULL, a_pOldRegs);
	/* Revert the bytes we modified. */
	for(itr=0;itr<a_itersCount;++itr){
		ptrace(PTRACE_POKETEXT, (pid_t)a_pid, (void *)(a_addr + 8*itr), (void *)a_save[itr]);
	}
}
...
static inline void* CInternalLoadLibOnRemoteProcessAndGetModuleInline(int a_pid, const char* a_libraryName,  int a_flag)
{
	...
	//const unsigned char injectCode[] = {0xff,0xd0,0x48,0x89,0xd9};
	unsigned char injectCode[] = {0x0,0x0,0x0,0x0,  // just a placeholder
								  0xff,0xd0, // call rax (and rax is dlopen)
								  0x48,0xbb,0x0,0x0,0x0,0x0,0x0,0x0,0x0,0x0,   // mov rbx, imediate
								  0x48, 0x89, 0xd8,   // mov rip, rbx   // set rax value to rip
								  0xcd,0x03,  // insert a breakpoint (finally we will jump here)
								  0x0,0x0,0x0  // dummy code
								 };
	...
	((unsigned long long int*)injectCode)[1] = fn_addr + 19; // this means we jump to the call rax next instruction
    ...
    /* Replace the bytes */
    if(!WriteDataToProcess(a_pid,fn_addr,injectCode+4,injectCodeSize-4)){
        ptrace(PTRACE_DETACH, (pid_t)a_pid, CPPUTILS_NULL, CPPUTILS_NULL);
        return CPPUTILS_NULL;
    }
    ...
    if (ptrace(PTRACE_SINGLESTEP, (pid_t)a_pid, CPPUTILS_NULL, CPPUTILS_NULL)) {
        RecoverDataAndRegsInline(a_pid,&oldregs,addr,itersCount,save);
        ptrace(PTRACE_DETACH, (pid_t)a_pid, CPPUTILS_NULL, CPPUTILS_NULL);
        return CPPUTILS_NULL;
    }
	...
	if(ptrace(PTRACE_GETREGS, (pid_t)a_pid, CPPUTILS_NULL, &regs)){
		RecoverDataAndRegsInline(a_pid,&oldregs,addr,itersCount,save);
        ptrace(PTRACE_DETACH, (pid_t)a_pid, CPPUTILS_NULL, CPPUTILS_NULL);
        return CPPUTILS_NULL;
	}
	
	while((regs.rbx==0) || (regs.rip<fn_addr) || (regs.rip>(fn_addr+19))){
		/* Execute inserted code */
		if (ptrace(PTRACE_SINGLESTEP, (pid_t)a_pid, CPPUTILS_NULL, CPPUTILS_NULL)) {
			RecoverDataAndRegsInline(a_pid,&oldregs,addr,itersCount,save);
			ptrace(PTRACE_DETACH, (pid_t)a_pid, CPPUTILS_NULL, CPPUTILS_NULL);
